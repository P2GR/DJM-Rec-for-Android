...
class RemoteDbClient(private val observer: WireObserver = WireObserver.NONE, private val connect: (String, Int) -> Socket) {
    fun query(address: String, ourNumber: Int, key: TrackKey): TrackMetadata? {
        ...
        val port = connect(address, 12523).use { socket ->
            val out = DataOutputStream(socket.getOutputStream().observed(observer, address, socket.port))
            out.writeInt(15); out.write("RemoteDBServer\u0000".toByteArray(Charsets.US_ASCII)); out.flush()
            DataInputStream(socket.getInputStream().observed(observer, address, socket.port)).readUnsignedShort()
        }
        require(port in 1..65535) { "Player has no metadata server" }
        return connect(address, port).use { socket ->
            val input = socket.getInputStream().observed(observer, address, socket.port)
            val output = socket.getOutputStream().observed(observer, address, socket.port)
